...
export class WebCanvasDriver implements ICanvasDriver {
  public name = 'web-html5-canvas';

  async loadImage(source: string): Promise<{ image: HTMLImageElement; width: number; height: number }> {
  ...
  async createOffscreenCanvas(width: number, height: number, dpr = 1): Promise<HTMLCanvasElement> {
  ...
  async render(canvas: HTMLCanvasElement, params: RenderParams): Promise<void> {
