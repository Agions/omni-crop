...
export class WechatCanvas2DDriver implements ICanvasDriver {
  public name = 'wechat-canvas-2d';

  /**
   * Preloads image via wx.downloadFile or wx.getImageInfo
   */
  async loadImage(source: string): Promise<{ image: any; width: number; height: number }> {
  ...
  async createOffscreenCanvas(width: number, height: number, dpr = 1): Promise<any> {
  ...
  async export(canvas: any, options: ExportOptions): Promise<CropResult> {
