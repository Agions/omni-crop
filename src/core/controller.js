import { getInitialCropSize, getMediaBaseSize, computeCropArea, } from './matrix/affine';
import { getCropBoundaries, clampPosition, getMinZoom, } from './boundary/restrict';
export class OmniCropController {
    ...
    initDimensions(containerSize, naturalMediaSize) {
        ...
        this.state.cropSize = getInitialCropSize(containerSize.width, containerSize.height, this.options.aspect);
        // 2. Calculate Base Rendered Media Size
        this.renderedMediaSize = getMediaBaseSize(naturalMediaSize.width, naturalMediaSize.height, this.state.cropSize.width, this.state.cropSize.height);
        ...
        if (this.options.restrictPosition) {
            const minRequired = getMinZoom(this.state.cropSize, this.state.mediaSize, this.state.rotation);
     ...
     */
    computeResult() {
        return computeCropArea(this.state.crop, this.state.cropSize, this.state.zoom, this.state.rotation, this.state.flip, this.naturalMediaSize, this.renderedMediaSize);
    }
    notifyComplete() {
    ...
    clampAndNotify() {
        if (this.options.restrictPosition && this.state.cropSize.width > 0) {
            const bounds = getCropBoundaries(this.state.cropSize, this.state.mediaSize, this.state.rotation, this.state.zoom);
